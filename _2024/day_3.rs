...
fn is_valid_instruction(_part: String) -> Option<bool> {
    None
}
...
fn extract_instructions(data: String) -> Vec<String> {
    ...
    let valid_instruction = is_valid_instruction(current_instruction);

    if valid_instruction.is_some() {
        println!("Instruction is Valid");
    }
