...
fn get_common_factors(number: usize) -> Vec<usize> {
    (1..number).filter(|i| number.is_multiple_of(*i)).collect()
}
...
fn get_invalid_id_sum(id_ranges: &str) -> i64 {
    ...
    for id_range in ranges {
        ...
        let (start_index, end_index) = (id_range.first().unwrap(), id_range.get(1).unwrap());
        let (start_index, end_index) = (
            start_index.parse::<i64>().unwrap(),
            end_index.parse::<i64>().unwrap(),
        );
...
fn main() {
    let content =
        fs::read_to_string("_2025/data/day_2_input.txt").expect("Could not Read Input File");
