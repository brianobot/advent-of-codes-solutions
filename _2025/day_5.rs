...
fn get_total_available_fresh_items(fresh_ids: Vec<String>, available_ids: Vec<String>) -> i32 {
    ...
    for available_id in available_ids {
        ...
        for fresh_id in fresh_ids.iter() {
            ...
            let start = start
                .parse::<i32>()
                .expect("Failed to convert start to integer");
            let end = end
                .parse::<i32>()
                .expect("Failed to convert start to integer");
            let range = start..=end;
...
fn main() {
    let binding =
        fs::read_to_string("_2025/data/day_5_input.txt").expect("Failed to Read Data File");
