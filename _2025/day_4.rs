...
fn get_neighbors(grid: Vec<Vec<char>>, (x_pos, y_pos): (i32, i32)) -> Vec<char> {
    ...
    for (dr, dc) in offsets {
        ...
        if (0 <= (new_row) && (new_row) < (row_len as i32))
            && (0 <= (new_col) && (new_col) < (col_len as i32))
        {
...
fn main() {
    let binding =
        fs::read_to_string("_2025/data/day_4_input.txt").expect("Could not read the input file");
